...
# Description
The following diagram visualizes the inputs and steps for our PlanetScope image processing pipeline, including the use of autoRIFT (code in a separate repository called [CautoRIFT](https://github.com/jukesliu/CautoRIFT)):
...
georeferencing the output velocity map. The DEM can be of any spatial resolution (the higher the better) because it will be resampled in the `custom_geogrid_autorift_opt` step in the [CautoRIFT](https://github.com/jukesliu/CautoRIFT)) repo.
...
│   ├── yyyy-mm                # Sub-directory with one month of imagery (auto-generated in `planetAPI_image_download` script)
│   ├── yyyy-mm                # Sub-directory with one month of imagery (auto-generated in `planetAPI_image_download` script)
│   │   ├── standard_grid      # Folder that holds all the 5m resampled imagery (auto-generated in `standardize_grid`)
│   │   ├── stitched_by_sat    # Folder that holds the along-track mosaicked images (auto-generated in `stich_by_sat`)
│   │   ├── stitched_imgs      # Folder that holds the final images for each acquisition date (auto-generated in `stitch_satswaths_by_date` or `select_largest_satswath`)
│   │   └── ...
│   └── ...
└── ...
```
...
```
sh run_pipeline_monthly.sh 2021-06 2021-12 /path/to/WGS/projection/AOI/shapefile.shp /path/to/UTM/projection/AOI/shapefile_UTM.shp /path/to/5m/raster.tif /path/to/glacier/outline.shp /base/path/ INSERT_PLANET_API_KEY /path/to/final/directory/to/store/noncloudy/images/
```
